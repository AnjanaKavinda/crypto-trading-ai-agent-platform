...
VALID_DATABASE_URL = (
    "postgresql+asyncpg://db_user:db_password@example.internal:5432/trading_platform"
)
...
def test_persistence_imports_and_application_creation_do_not_require_database_url(
    ...
    persistence_session_module = importlib.import_module(
        "trading_platform_api.persistence.session"
    )

    def fail_engine_creation(*args, **kwargs):
        raise AssertionError(
            "application import and creation must not construct a database engine"
        )

    monkeypatch.setattr(
        persistence_session_module, "create_async_engine", fail_engine_creation
    )
...
def test_engine_and_session_factories_use_async_stack_without_connecting(
    ...
    engine = create_async_engine_instance(
        load_database_settings({"DATABASE_URL": VALID_DATABASE_URL})
    )
...
def test_failed_transaction_scope_rolls_back_closes_and_reraises_original_exception() -> (
    None
):
    session = FakeSession()
...
def test_persistence_package_introduces_foundation_only_and_no_repository_crud_exports() -> (
    None
):
    ...
    assert not any(
        "repository" in exported_name.lower()
        for exported_name in persistence_module.__all__
    )
